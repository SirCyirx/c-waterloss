...
AddEventHandler('playerSwimming', function()
    ...
    if Config.Inventory == 'qb' then
        local hasphone = QBCore.Functions.HasItem("phone", 1)
        local hasradio = QBCore.Functions.HasItem("radio", 1)
    ...
    elseif Config.Inventory == 'lj' then
        local hasphone = exports['lj-inventory']:HasItem('phone', 1)
        local hasradio = exports['lj-inventory']:HasItem('radio', 1)
        if hasphone then
            TriggerServerEvent('c-waterloss:server:takeinwaterphone')
            givebackphone = true
